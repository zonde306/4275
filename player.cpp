...
void CBasePlayer::SetScoreAttrib(CBasePlayer *dest)
{
	...
	if (pev->deadflag != DEAD_NO)
		state  |= SCOREATTRIB_DEAD;

	if (m_bHasC4)
	{
		if (dest->m_iTeam == m_iTeam || dest->m_iTeam == TEAM_SPECTATOR)
			state  |= SCOREATTRIB_BOMB;
	}

	if (m_bIsVIP == true)
		state  |= SCOREATTRIB_VIP;
...
void CBasePlayer::SelectLastItem(void)
{
	...
	{
		for (int i = 1; i < MAX_ITEM_TYPES - 1; i++)
		{
...
void CBasePlayer::AddAutoBuyData(const char *string)
{
	...
	if (len < MAX_AUTOBUY_LENGTH - 1)
	{
		...
		strncat(m_autoBuyString, string, MAX_AUTOBUY_LENGTH - len);
	}
}
