...
{
	if ((int)startmoney.value > 65535)
		CVAR_SET_FLOAT("mp_startmoney", 65535);
	else if ((int)startmoney.value < 0)
...
void ClientPutInServer(edict_t *pEntity)
{
	...
	if (g_pGameRules && g_pGameRules->m_bMapHasCameras)
...
BOOL HandleMenu_ChooseTeam(CBasePlayer *pPlayer, int slot)
{
	...
	if (pPlayer->m_bIsVIP == true)
	{
		...
		if (g_pGameRules->IsVIPQueueEmpty() == TRUE)
		{
			ClientPrint(pPlayer->pev, HUD_PRINTCENTER, "#Cannot_Switch_From_VIP");
			CLIENT_COMMAND(ENT(pPlayer->pev), "slot10\n");
			return TRUE;
...
__declspec(naked) void ClientCommand(edict_t *pEntity)
{
	__asm
	{
		mov dl, byte ptr [UseBotArgs]
		jmp _ClientCommand
		nop
		nop
		nop
...
void ResetPlayerPVS(edict_t *client, int clientnum)
{
	...
	g_PVSStatus->headnode = client->headnode;
	g_PVSStatus->num_leafs = client->num_leafs;
	memcpy(g_PVSStatus->leafnums, client->leafnums, sizeof(g_PVSStatus->leafnums));
}
