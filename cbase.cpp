...
int(*g_oDispatchSpawn)(edict_t*) = nullptr;
int Hooked_DispatchSpawn(edict_t *pent)
{
	if (strcmp(STRING(pent->v.classname), "worldspawn"))
	{
...
#else
		mkdir("czero", 777);
		mkdir("czero/maps", 777);
#endif
	}

	return g_oDispatchSpawn(pent);
}
...
int GetEntityAPI(DLL_FUNCTIONS *pFunctionTable, int interfaceVersion)
{
	...
	g_oDispatchSpawn = pFunctionTable->pfnSpawn;
	pFunctionTable->pfnSpawn = Hooked_DispatchSpawn;
